...
const HomePage = () => {
        ...
        const getAllContent = async () => {
            try {
                ...
                const [videoRes, tweetRes] = await Promise.allSettled([
                    fetchData("GET", "http://localhost:5000/api/v1/Videos"),
                    fetchData("GET", "http://localhost:5000/api/v1/tweets")
                ]);
    ...
    const handleLikeTweet = async (tweetId, type) => {
        ...
        try {
            await fetchData("POST", `http://localhost:5000/api/v1/Videos/Likes/${tweetId}`, { type });
        } catch(e) {
    ...
    const handleDeleteTweet = async (tweetId) => {
        ...
        try {
            await fetchData("DELETE", `http://localhost:5000/api/v1/tweets/delete/${tweetId}`);
        } catch (e) {
            setTweets(previousTweets);
