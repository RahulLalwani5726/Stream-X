...
const AuthPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        // --- LOGIN ---
        const response = await fetchData("POST", "https://stream-x.onrender.com/api/v1/users/login", {
        ...
        const response = await fetchData("POST", "https://stream-x.onrender.com/api/v1/users/register", apiFormData);
        const data = response.Data || response.data;
