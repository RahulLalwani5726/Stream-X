...
const TweetsPage = () => {
  ...
  const getTweets = async () => {
    try {
      const response = await fetchData(
        "GET",
        "https://stream-x.onrender.com/api/v1/tweets"
      );
  ...
  const handlePostTweet = async (content, imageFile) => {
    ...
    try {
      ...
      await fetchData(
        "POST",
        "https://stream-x.onrender.com/api/v1/tweets/create",
        formData,
  ...
  const handleDeleteTweet = async (tweetId) => {
    ...
    try {
      await fetchData(
        "DELETE",
        `https://stream-x.onrender.com/api/v1/tweets/delete/${tweetId}`
      );
    } catch (e) {
  ...
  const handleLikeTweet = async (tweetId, type) => {
    ...
    try {
      await fetchData(
        "POST",
        `https://stream-x.onrender.com/api/v1/Videos/Likes/${tweetId}`,
        { type }
      );
    } catch (e) {
