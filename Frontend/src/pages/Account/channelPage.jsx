...
const ChannelPage = () => {
    ...
    const loadChannel = async () => {
      try {
        ...
        const profileRes = await fetchData(
          "GET",
          `http://localhost:5000/api/v1/users/channel/${username}`
        );
        ...
        const [vidRes, twtRes, plRes] = await Promise.allSettled([
          fetchData(
            "GET",
            `http://localhost:5000/api/v1/Videos/get-channel-videos/${cleanProfile._id}`
          ),
          fetchData(
            "GET",
            `http://localhost:5000/api/v1/tweets/get-channel-tweets/${cleanProfile._id}`
          ),
          fetchData(
            "GET",
            `http://localhost:5000/api/v1/playlist/user/${cleanProfile._id}`
          ),
        ]);
  ...
  const handleSubscribe = async () => {
    ...
    try {
      await fetchData(
        "POST",
        `http://localhost:5000/api/v1/users/subscribe/${channel.username}`
      );
    } catch (e) {
      setChannel(oldState);
    } finally {
