...
const SettingsModal = ({ user, onClose, onSave }) => {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      await fetchData(
        "DELETE",
        "https://stream-x.onrender.com/api/v1/users/delete-account"
      );
      localStorage.clear();
...
const Account = () => {
    ...
    try {
      ...
      const [videoRes, tweetRes, playlistRes, historyRes, subsCountRes] =
        await Promise.allSettled([
          fetchData(
            "GET",
            "https://stream-x.onrender.com/api/v1/Videos/get-user-video-list"
          ),
          fetchData(
            "GET",
            "https://stream-x.onrender.com/api/v1/tweets/get-user-tweet-list"
          ),
          fetchData(
            "GET",
            `https://stream-x.onrender.com/api/v1/playlist/user/${currentUser._id}`
          ),
          fetchData("GET", "https://stream-x.onrender.com/api/v1/users/history"),
          fetchData(
            "GET",
            "https://stream-x.onrender.com/api/v1/users/subscribers/count"
          ),
        ]);
  ...
  const handleDelete = async (id, type) => {
    ...
    try {
      let endpoint = "";
      if (type === "video")
        endpoint = `https://stream-x.onrender.com/api/v1/Videos/delete/${id}`;
      if (type === "tweet")
        endpoint = `https://stream-x.onrender.com/api/v1/tweets/${id}`;
      if (type === "playlist")
        endpoint = `https://stream-x.onrender.com/api/v1/playlist/${id}`;
  ...
  const handleUpdateItem = async (id, data, type) => {
    try {
      let endpoint = "";
      if (type === "video")
        endpoint = `https://stream-x.onrender.com/api/v1/Videos/updatefields/${id}`;
      else if (type === "tweet")
        endpoint = `https://stream-x.onrender.com/api/v1/tweets/update/${id}`;
      else endpoint = `https://stream-x.onrender.com/api/v1/playlist/edit/${id}`;
  ...
  const handleUploadItem = async (formData, type) => {
    try {
      let endpoint = "";
      if (type === "video")
        endpoint = "https://stream-x.onrender.com/api/v1/Videos/upload";
      else if (type === "tweet")
        endpoint = "https://stream-x.onrender.com/api/v1/tweets/create";
  ...
  const handleSaveSettings = async (data) => {
    ...
    try {
      if (
        ...
        if (data.fullname !== user.fullname || data.email !== user.email) {
          await fetchData(
            "PATCH",
            "https://stream-x.onrender.com/api/v1/users/update-account",
        ...
        if (data.avatarFile) {
          ...
          const avRes = await fetchData(
            "PATCH",
            "https://stream-x.onrender.com/api/v1/users/avatar",
            avatarData
          );
    ...
    if (data.oldPassword && data.newPassword) {
      try {
        await fetchData(
          "POST",
          "https://stream-x.onrender.com/api/v1/users/change-password",
  ...
  const handleCoverImageChange = async (e) => {
    ...
    try {
      ...
      const res = await fetchData(
        "PATCH",
        "https://stream-x.onrender.com/api/v1/users/cover-image",
        formData
      );
      if (res?.Data) {
  ...
  const handleLogout = async () => {
    try {
      await fetchData("POST", "https://stream-x.onrender.com/api/v1/users/logout");
      localStorage.clear();
      window.location.href = "/auth";
    } catch (e) {
