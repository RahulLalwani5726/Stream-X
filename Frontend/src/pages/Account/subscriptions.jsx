...
const UserCard = ({ user, type }) => {
    ...
    const handleToggleSubscribe = async () => {
        ...
        try {
            await fetchData("POST", `http://localhost:5000/api/v1/users/subscribe/${user.username}`);
        } catch (e) {
        ...
        const getData = async () => {
            ...
            try {
                const endpoint = activeTab === "subscribers" 
                    ? "http://localhost:5000/api/v1/users/subscribers/list" 
                    : "http://localhost:5000/api/v1/users/subscription/list"; 
