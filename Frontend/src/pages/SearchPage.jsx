...
const SEARCH_ENDPOINT_BASE = "https://stream-x.onrender.com/api/v1/public/search";
...
const UserSearchCard = ({ user, onSubscribe }) => {
  ...
  return (
    <div
      ...
      onClick={() => navigate(`/account/channel/${user.username}`)}
    >
      <img
        src={user.avatar}
        alt={user.username}
...
const SearchPage = () => {
  ...
  const handleSubscribe = async (targetUser) => {
    ...
    try {
      await fetchData(
        "POST",
        `https://stream-x.onrender.com/api/v1/users/subscribe/${targetUser.username}`
      );
    } catch (e) {
