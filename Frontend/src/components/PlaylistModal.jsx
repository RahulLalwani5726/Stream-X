...
const PlaylistModal = ({ onClose, videoId, userData }) => {
  ...
  useEffect(() => {
    const fetchUserPlaylists = async () => {
      try {
        setLoading(true);
        const res = await fetchData("GET", `http://localhost:5000/api/v1/playlist`);
      ...
      try {
          ...
          playlists.forEach(playlist => {
              ...
              if (isCurrentlySelected && !wasInitiallySelected) {
                  promises.push(
                      fetchData("PATCH", `http://localhost:5000/api/v1/playlist/add/${playlist._id}/${videoId}`)
                  );
              }
              ...
              if (!isCurrentlySelected && wasInitiallySelected) {
                  promises.push(
                      fetchData("PATCH", `http://localhost:5000/api/v1/playlist/remove/${playlist._id}/${videoId}`)
                  );
              }
          });
  ...
  const handleCreatePlaylist = async (e) => {
    ...
    try {
        const res = await fetchData("POST", "http://localhost:5000/api/v1/playlist/create", {
        ...
        if(createdPlaylist) {
             ...
             await fetchData("PATCH", `http://localhost:5000/api/v1/playlist/add/${createdPlaylist._id}/${videoId}`);
