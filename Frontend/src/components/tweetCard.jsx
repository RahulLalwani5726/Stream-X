...
const TweetCommentsSection = ({ tweetId, userData, onCommentChange }) => {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            setIsLoading(true);
            try {
                const res = await fetchData("GET", `http://localhost:5000/api/v1/Videos/comment/${tweetId}`);
    ...
    const handleAddComment = async (type, content, targetId) => {
        ...
        try {
            ...
            await fetchData("POST", `http://localhost:5000/api/v1/tweets/comment/create/${targetId}`, { content, type });
            
            // Sync with Server (Optional, keeps IDs correct)
            const res = await fetchData("GET", `http://localhost:5000/api/v1/tweets/comment/${tweetId}`);
            if (res?.Data) setComments(res.Data);
    ...
    const handleDeleteComment = async (commentId) => {
        ...
        try {
            await fetchData("DELETE", `http://localhost:5000/api/v1/tweets/comment/delete/${commentId}`);
        } catch (e) {
            // Revert on error
    ...
    const handleUpdateComment = async (commentId, newContent) => {
        ...
        try {
            await fetchData("PATCH", `http://localhost:5000/api/v1/tweets/comment/edit/${commentId}`, { content: newContent });
